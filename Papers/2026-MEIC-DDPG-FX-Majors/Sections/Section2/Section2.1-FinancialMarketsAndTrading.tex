...
These charges do not apply uniformly across account types, and the choice of account is therefore part of the design of a strategy rather than an administrative detail. A raw-spread account quotes the interbank spread with no markup and recovers its margin through the commission, which makes the total cost of a position lower and, more usefully, explicit: the implicit part shrinks to the market spread and the rest is a published rate. A swap-free account charges no overnight financing at all. The two together suit a strategy that holds positions for days rather than seconds, because such a strategy pays the spread and the commission only a few times per position but would otherwise pay financing on every night the position stays open. This work evaluates on an account that is both, so the spread and the commission are the only charges applied, and the financing term is genuinely zero rather than estimated.
...
\label{Subsection:PerformanceMeasurement}

A trading strategy is judged by what it earns and by the risk taken to earn it, so a single return figure is not enough to compare two strategies. The measures below are the ones reported in Section~\ref{Section:ExperimentalResults}. They are defined on an equity series and therefore apply unchanged to a portfolio of several strategies, computed on the combined equity rather than on each strategy separately, which matters because a portfolio's risk depends on how its components move together and not only on their individual risks.
